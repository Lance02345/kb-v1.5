...
<main class="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <section class="grid gap-6 lg:grid-cols-3">
        <article class="space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-5 lg:col-span-2">
            <h1 class="font-display text-2xl font-bold text-white">{{ $vehicle->title }} {{ $vehicle->carmodel?->carmake?->make }} {{ $vehicle->carmodel?->model }}</h1>
            ...
            <div class="grid gap-2 text-sm text-slate-300 sm:grid-cols-2 lg:grid-cols-3">
                <p class="rounded-lg border border-slate-800 bg-slate-950/40 p-2">Price: <span class="text-white">Ksh {{ number_format((float) $vehicle->price) }}</span></p>
                <p class="rounded-lg border border-slate-800 bg-slate-950/40 p-2">Mileage: <span class="text-white">{{ number_format((int)$vehicle->mileage) }} km</span></p>
                ...
                <p class="rounded-lg border border-slate-800 bg-slate-950/40 p-2">Body: <span class="text-white">{{ $vehicle->body_type }}</span></p>
                <p class="rounded-lg border border-slate-800 bg-slate-950/40 p-2">Views: <span class="text-white">{{ number_format((int) $vehicle->views) }}</span></p>
            </div>
